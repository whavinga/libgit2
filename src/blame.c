		...
		git_blame *blame)
{
	...
	size_t wedge_line = (range->old_lines == 0) ? range->new_start : range->old_start;
	blame_hunk *hunk;
	...
	if (hunk) {
		...
		if (!line_is_at_start_of_hunk(wedge_line, hunk)) {
			hunk = split_hunk_in_vector(&blame->unclaimed_hunks, hunk, wedge_line, true);
		}
	}
